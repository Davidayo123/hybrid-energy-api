...
class RealTimeHybridForecaster:
    ...
    def __init__(self, gru_model_path='te_gru_custom.keras', lgb_model_path='lightgbm_baseline.pkl'):
        ...
        print(f"DEBUG: Checking path: {gru_full_path}")

        # 1. Load the Pre-Trained Base Models
        try:
            self.te_gru = load_model(gru_full_path)
            self.lgb_model = joblib.load(lgb_full_path)
            print("✅ Models loaded successfully from absolute paths.")
        except Exception as e:
            print(f"❌ FAILED TO LOAD MODELS: {str(e)}")
            # Instead of setting to None, we raise an error to stop the server from starting broken
            raise RuntimeError(f"Could not load models. Check if {gru_model_path} exists on GitHub.")
        
        # 2. Hardcode the Champion 7D "God-Mode" Parameters...
        # (Keep the rest of your window_size, mh_steps, etc. exactly as they are)
